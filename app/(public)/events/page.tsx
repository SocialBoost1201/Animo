            ...
            return (
              <FadeIn key={item.id} delay={index * 0.05} className="group cursor-pointer">
                ...
                <Link href={`/events/${item.id}`} className="block">
                  <div className="flex flex-col md:flex-row bg-white hover:bg-gray-50/30 p-8 md:p-12 shadow-luxury transition-all duration-700 border border-[var(--color-gold)]/20 hover:-translate-y-1">
                    <div className="shrink-0 mb-6 md:mb-0 md:mr-12 text-[10px] font-serif luxury-tracking text-[#171717] w-48 flex items-center md:pt-1">
                      <Calendar size={14} className="mr-3 text-[var(--color-gold)] opacity-70" />
                      {eventDateStr ? `${eventDateStr} 開催` : '開催日未定'}
                    </div>
                    
                    <div className="flex-1">
                      <h3 className="text-lg font-serif luxury-tracking text-[#171717] mb-4 group-hover:text-[var(--color-gold)] transition-colors line-clamp-2">
                        {item.title}
                      </h3>
                      {item.description && (
                    ...
                    <div className="shrink-0 flex items-center mt-6 md:mt-0 text-[var(--color-gold)] opacity-0 group-hover:opacity-100 transition-all md:-translate-x-4 group-hover:translate-x-0 duration-700">
