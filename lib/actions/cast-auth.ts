...
async function findCastIdentityByPhone(serviceRoleClient: ReturnType<typeof createServiceClient>, phoneInput: string) {
  const normalizedPhone = normalizeJapaneseMobilePhoneForMatch(phoneInput);
  if (!normalizedPhone) {
    return { data: [] as CastIdentityCandidate[], error: null };
  }
