...
export async function DashboardReservations() {
  ...
  return (
    <div className="flex flex-col bg-[#17181c] rounded-[18px] overflow-hidden border border-[#8a8478] font-inter h-full">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between px-6 h-[72px] border-b border-[#8a847830] gap-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <p className="text-[13px] font-semibold text-[#f4f1ea] tracking-[-0.08px] leading-tight">来店予定一覧</p>
        ...
        <div className="flex items-center gap-2">
          {confirmedCount > 0 && (
            <div className={`hidden sm:flex items-center px-2 py-1 ${STATUS_CONFIG.confirmed.bg} rounded-[8px] h-[30px]`}>
              <span className={`text-[10px] font-semibold whitespace-nowrap tracking-[0.12px] ${STATUS_CONFIG.confirmed.textColor}`}>確認済 {confirmedCount}</span>
            </div>
          )}
          {unconfirmedCount > 0 && (
            <div className={`hidden sm:flex items-center px-2 py-1 ${STATUS_CONFIG.unconfirmed.bg} rounded-[8px] h-[30px]`}>
              <span className={`text-[10px] font-semibold whitespace-nowrap tracking-[0.12px] ${STATUS_CONFIG.unconfirmed.textColor}`}>要確認 {unconfirmedCount}</span>
            </div>
          )}
          <Link
            href="/admin/today"
      ...
      <div className="flex-1 overflow-x-auto custom-scrollbar">
        <div className="min-w-[800px]">
          ...
          <div className="flex items-center h-[42px] border-t border-b border-[#ffffff0a] px-6">
            ...
            <div className="flex-1 text-[9px] font-bold tracking-[0.71px] text-[#5a5650] uppercase px-4">お客様名</div>
            ...
            {reservations.length === 0 ? (
              ...
              reservations.map((r) => {
                ...
                return (
                  ...
                  >
                    {/* Time */}
                    <div className="w-section-mobile text-[12px] font-semibold text-[#dfbd69]">{r.visitTime}</div>
                    
                    {/* Guest Name */}
                    <div className="flex-1 text-[12px] font-semibold text-[#cbc3b3] truncate px-4">
                      {r.guestName} 様
                    </div>
                    
                    {/* Guest Count */}
                    <div className="w-[60px] text-[12px] font-medium text-[#8a8478] text-center">
                      <span className="text-[#c7c0b2]">{r.guestCount}</span>名
                    </div>
                    ...
                    <div className="w-[180px] px-4 overflow-hidden">
                      <div className="flex items-center gap-2">
                        {r.castName !== '—' ? (
                          <>
                            <div className="w-5 h-5 rounded-full bg-[#dfbd691a] flex items-center justify-center shrink-0">
                    ...
                    <div className="w-[150px] px-4 overflow-hidden">
                      <p className="text-[11px] text-[#5a5650] truncate" title={r.note ?? ''}>
                        {r.note || '—'}
                      </p>
                    </div>
                  </div>
