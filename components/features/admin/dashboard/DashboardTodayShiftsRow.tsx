...
import type { DashboardCastShift } from '@/lib/actions/dashboard'
...
// ──────────────────────────────────────────────────────
// Row component (read-only; deep attendance edits live on /admin/today)
// ──────────────────────────────────────────────────────
export function DashboardTodayShiftsRow({ cast }: { cast: DashboardCastShift }) {
  ...
  return (
    <div className="flex items-center min-h-[60px] hover:bg-white/2 transition-colors px-6">
      {/* Cast name */}
      ...
      <div className="w-[220px] shrink-0 flex flex-col gap-2 py-2">
        ...
        <Link
          href="/admin/today"
          prefetch={false}
          className="text-[10px] font-bold text-[#dfbd69] hover:underline self-start"
        >
          本日の出勤で調整
        </Link>
      </div>
