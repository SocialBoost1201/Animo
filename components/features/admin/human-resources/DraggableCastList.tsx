  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: '#2a2112', boxShadow: '2px 4px 14px rgba(0,0,0,0.46)' }}
      />
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(180deg, #191715 0%, #111111 42%, #0d0d0d 100%)',
          border: '1px solid rgba(223, 189, 105, 0.14)',
        }}
      />
      ...
      <div className="flex justify-between items-start md:hidden p-4 pb-0">
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-2 rounded-lg text-[#b7af9f] hover:text-[#dfbd69] hover:bg-white/5 transition-colors"
          >
        ...
        <div className="flex items-center gap-1">
          <Link
            ...
            className="p-2 text-[#b7af9f] hover:text-[#dfbd69] transition-all rounded-lg hover:bg-white/5"
          >
            <Edit2 size={18} />
          </Link>
      ...
      <div className={`flex flex-col md:grid ${GRID_COLS} md:items-center gap-4 md:gap-0 p-4 md:p-0 md:px-5 md:py-4`}>
        ...
        <div className="hidden md:flex items-center gap-2 pl-1">
          ...
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-1.5 rounded-lg text-[#b7af9f] hover:text-[#dfbd69] hover:bg-white/5 transition-all opacity-45 group-hover:opacity-100"
          >
        ...
        <div className="flex items-center gap-4 md:contents">
          ...
          <div className="flex-1 min-w-0 md:hidden">
            ...
            {cast.hobby && (
              <p className="text-[12px] text-[#b7af9f] truncate mt-0.5">{cast.hobby}</p>
            )}
          </div>
        </div>
        ...
        <div className="hidden md:block min-w-0 pl-3">
          ...
          {cast.hobby && (
            <p className="text-[12px] text-[#c7c0b2] truncate mt-0.5">{cast.hobby}</p>
          )}
        </div>

        {/* Age */}
        <div className="hidden md:block text-[14px] font-semibold text-[#e6dfd1] pl-2">
          {cast.age ? `${cast.age}歳` : <span className="text-[#978d7d]">—</span>}
        </div>
        ...
        <div className="flex md:flex-col items-center md:items-start justify-between md:justify-center gap-2 md:gap-1.5 py-3 md:py-0 border-t border-[#ffffff06] md:border-0 pl-0 md:pl-1">
          <span className="md:hidden text-[10px] font-bold text-[#978d7d] tracking-widest">SCORE</span>
          <div className="flex flex-col md:gap-1.5 items-end md:items-start">
            <span className="font-bold text-[18px] md:text-[16px] flex items-center gap-1.5" style={goldGradientStyle}>
              ...
              <span className="text-[10px] text-[#a89d8a] tracking-wider uppercase font-sans font-black" style={{ WebkitTextFillColor: '#a89d8a', background: 'none' }}>pt</span>
            </span>
            <span className="text-[10px] font-black px-2 py-0.5 rounded-md flex items-center gap-1 w-fit border bg-[#dfbd69]/12 text-[#f0d898] border-[#dfbd69]/28">
        ...
        <div className="flex md:block items-center justify-between py-2 md:py-0 md:pl-1">
          <span className="md:hidden text-[10px] font-bold text-[#978d7d] tracking-widest">TAGS</span>
          {cast.quiz_tags && cast.quiz_tags.length > 0 ? (
            <div className="flex items-center gap-1.5 text-[#e2ddd4]">
              <Sparkles size={13} className="text-[#dfbd69]/60 shrink-0" />
              <span className="text-[13px] font-bold">
                {cast.quiz_tags.length}
                <span className="font-normal text-[#b7af9f] hidden lg:inline ml-1">項目設定済み</span>
              </span>
            </div>
          ) : (
            <span className="text-[12px] text-[#978d7d] flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-[#ffffff12]" />
              未設定
            </span>
          )}
        </div>
        ...
        <div className="flex md:block items-center justify-between py-2 md:py-0">
          <span className="md:hidden text-[10px] font-bold text-[#978d7d] tracking-widest">STATUS</span>
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.8px] transition-all ${
              isPublic
                ...
                : 'bg-[#1a1c22] text-[#c7c0b2] border border-[#dfbd69]/14'
            }`}
          >
            {isPublic ? (
        ...
        <div className="hidden md:flex justify-end items-center gap-1 pr-2">
          <Link
            href={`/admin/human-resources/${cast.id}`}
            className="p-2 text-[#b7af9f] hover:text-[#dfbd69] transition-all duration-200 rounded-lg hover:bg-white/5"
            title="編集"
          >
            <Edit2 size={16} />
          </Link>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 min-h-[40px]">
        ...
        >
          <span className="text-[12px] font-bold text-[#c7c0b2] border-r border-[#dfbd69]/16 pr-4 mr-1">
          ...
          <button
            ...
            disabled={isPending}
            className="flex items-center gap-1.5 px-3.5 py-1.5 text-[11px] font-bold text-[#e2ddd4] bg-white/4 border border-[#dfbd69]/16 rounded-lg hover:bg-white/8 transition-colors"
          >
        ...
        {!showBulkActions && (
          <button
            onClick={saveOrder}
            disabled={isPending}
            className="flex items-center justify-center gap-2 bg-[#18171400] border border-[#dfbd69]/22 hover:border-[#dfbd69]/45 text-[#e2ddd4] hover:text-[#dfbd69] px-5 py-2 text-[12px] font-bold tracking-wider transition-all duration-200 disabled:opacity-40 rounded-[10px] ml-auto shadow-[0_10px_24px_rgba(0,0,0,0.18)]"
          >
      ...
      <div className="w-full">
        ...
        {/* Desktop Header */}
        <div className={`hidden md:grid ${GRID_COLS} gap-0 px-5 py-3 mb-2 text-[10px] tracking-[0.12em] text-[#a89d8a] font-bold uppercase items-center border-b border-[#dfbd69]/14`}>
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <div className="space-y-2">
            ...
            {casts.length === 0 && (
              <div className="py-20 text-center rounded-[18px] border border-dashed border-[#dfbd69]/18 bg-[#0f0e0c]">
                ...
                <p className="text-[14px] font-bold text-[#e2ddd4]">対象のキャストが見つかりません</p>
                <p className="text-[12px] text-[#b7af9f] mt-1.5">検索条件を変更するか、新しくキャストを登録してください。</p>
              </div>
            )}
          </div>
        </DndContext>
